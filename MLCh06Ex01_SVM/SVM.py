...
def select_2rdAlpha(firstAlpha, sumOfAlpha):
    secondAlpha = firstAlpha
    while secondAlpha == firstAlpha:
        secondAlpha = int(random.uniform(0, sumOfAlpha))
    return secondAlpha


# 辅助函数，调整大于 highLimit和小于 lowLimit的 a
def adjust_alpha(alpha, highLimit, lowLimit):
    if alpha > highLimit:
        alpha = highLimit
    elif alpha < lowLimit:
        alpha = lowLimit
    ...
    while unchangedSum < maxCycle:
        ...
        for i in range(sampleNum):
            ...
            if ((Y_vec[i] * errorXi < -faultToler) and (0 <= Alpha_mat[i] <= c)) or \
                ...
                # 修正一下 aj，不能越界
                Alpha_mat[j] = adjust_alpha(Alpha_mat[j], highLimit, lowLimit)
                # 变化的太小则忽略这次变化
