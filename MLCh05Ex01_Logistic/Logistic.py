...
最大似然法：(X,W是包含一个样本中所有 xi和 wi的向量，我们假设有 m个样本)
  ...
  L(W) = P(y0|z0)*P(y1|z1)*...*P(yn|zn) = [(h(z0))^y0 * (1-h(z0))^(1-y0)]*...*[(h(zm))^ym * (1-h(zm))^(1-ym)]
  习惯性的取对数 l(W) = log(L(W)) = sum(yj*log(h(zj)) + (1-yj)*log(1-h(zj)))
  我们的目的是找到最大的 W 比较合适的方法是梯度上升，这里遵照博文用 Andrew Ng中的办法：
  取 J(W) = -(1/m)l(W) 换成求最小值，用梯度下降法

梯度下降法：wi：= Wi - α*(∂J(W)/∂wi)
  ∂J(W)/∂wi 的求解过程这里略过，具体参照博文
  最后的结果是∂J(W)/∂wi = (1/m) * sum(h(zj)-yj) *xji
  梯度算法的过程写为：wi: = wi- α*(1/m) * sum[(h(zj)-yj)*xji]
  α本来就是待定系数，所以把(1/m)省略掉，最后的 wj更新式为：
  wi: = wi - α * sum[(h(zj) - yj)*xji]  (α是个正数为前提)
  如果是用上升法那么是这样子的：
  wi: = wi + α * sum[(yj - h(Xj))*xji]  (α是个正数为前提)

上式是对单个 wj系数的求法，现在我们打算 n个系数一起求：
我们令 Xj = [xj0;xj1;...;xjn] (其中 xi0为 1) 为第 i个样本的属性向量(列向量) Z = [z1;z2;...;zm] 是 Z阵
      W = [w0;w1;...;wn] 是系数向量 Y = [y1;y2;...;ym] 是标签向量 H = [h1;h2;...;hm]是sigmoid矩阵
      A = [X1';X2',...,Xm'] 把一个个 X向量横起来写，比较像原始的数据样式
我们用梯度下降法：
  首先算 Z：Z = A * W 再算 H：H = h(Z)
  h(zj) - yj扩展到矩阵是：H - Y
  sum[(yj - h(zj))*xji]扩展到矩阵是：A'*(H - Y)
  综合起来就是：W: = W - α * A'*(H - Y)
